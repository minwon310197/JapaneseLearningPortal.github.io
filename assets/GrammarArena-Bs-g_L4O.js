...
import { Q as QuestionDisplay, A as AnswerOptionRow, a as QuizMode } from "./QuizMode-DQg4kJzy.js";
...
import { ap as shuffleArray$1, K as equalsAnswerText, x as playSFX, L as speakJP, F as content, u as useAppStore } from "./index-BEJSIlFS.js";
import { S as ScaffoldingLayer } from "./ScaffoldingLayer-BwAQh4aw.js";
import { Q as QuizFeedback } from "./QuizFeedback-Pap1aVYn.js";
import { M as ModeResultsScreen, s as safeTtsText } from "./ModeResultsScreen-5Gki9ifL.js";
...
import { a as useGrammarItems, b as useSectionList } from "./useDataHelper-DtTUT9Wk.js";
import { u as useGrammarGameItems } from "./useGrammarQuiz-CkAk5tho.js";
import { a as grammarAccessors } from "./index-B2ai8n3C.js";
...
const MODES = [
  ...
  { id: "scramble", label: "Sắp xếp câu", icon: "🔀" },
...
function GrammarSentenceBuildMode({ items, maxQuestions, difficulty }) {
  const scoring = useScoreEngine("grammar-arena");
  const questions = reactExports.useMemo(() => items.flatMap((item) => {
    const sentences = item.sentence ? [item.sentence] : extractExamples(item.content || "");
    return sentences.filter(Boolean).map((sentence) => ({ ...item, sentence, answer: "", grammar: item.title, vi: item.vi || item.meaning || "" }));
  }), [items]);
  return /* @__PURE__ */ jsxRuntimeExports.jsx(
    SentenceBuildMode,
    {
      ...
      maxQuestions,
      difficulty,
      onAnswerEvaluated: ({ question, isCorrect }) => isCorrect ? scoring.recordCorrect(question) : scoring.recordWrong(question),
      onFinish: () => scoring.reset()
    }
  );
}
function GrammarArena({ mode = "quiz" }) {
  ...
  const renderMode = () => {
    ...
    switch (mode) {
      ...
      case "scramble":
        return /* @__PURE__ */ jsxRuntimeExports.jsx(GrammarSentenceBuildMode, { items: hasGameItems ? quizItems : items, maxQuestions: maxQ, difficulty });
      default:
        return /* @__PURE__ */ jsxRuntimeExports.jsx(
          QuizMode,
          {
