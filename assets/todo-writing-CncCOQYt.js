import { bB as shuffleArray, bK as esc, s as safeGetItem, S as STORAGE_KEYS, d as safeSetItem, ab as S } from "./feature-3d-CFvJkEt3.js";
...
function openCanvasWriting() {
  ...
  h += '<div class="wr-info">' + esc(k.title || "") + " · " + esc(k.strokes || "?") + " nét</div>";
  h += "</div>";
...
function openRadicalDetail() {
  ...
  h += "</div>";
  h += '<div class="wr-extra"><b>On:</b> ' + esc(k.on || "") + " · <b>Kun:</b> " + esc(k.kun || "") + " · <b>Nét:</b> " + esc(k.strokes || "?") + "</div>";
...
function openComponentTree() {
  ...
  h += '<div class="wr-target"><div class="wr-kanji">' + esc(k.kanji) + "</div>";
  h += '<div class="wr-info">' + esc(k.title || "") + " · " + esc(k.strokes || "?") + " nét</div></div>";
  h += '<div class="wr-tree">';
