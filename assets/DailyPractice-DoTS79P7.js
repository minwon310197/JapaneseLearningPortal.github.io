...
import { Q as QuizMode } from "./QuizMode-CPaCpYew.js";
import "./MatchMode-x-8EHuqx.js";
import { T as TrueFalseMode } from "./TrueFalseMode-DFqC7BKW.js";
import { F as FlashcardMode } from "./FlashcardMode-BoBgQHca.js";
import { u as useVocabItems, a as useKanjiItems, b as useGrammarItems } from "./useDataHelper-Bybwc3O4.js";
import { u as useGrammarGameItems } from "./useGrammarQuiz-bSsnJV68.js";
import { a1 as makeVocabKey, a0 as makeKanjiKey, A as content } from "./feature-3d-hud-Dp6hMoyV.js";
import { bA as playSFX, f as speakJP, u as useLearningStore } from "./feature-3d-jK3b4Iv-.js";
...
function ReviewMode({ vocabItems, kanjiItems }) {
  const srs = useLearningStore((s) => s.srs);
  const srsSnapshotRef = reactExports.useRef(null);
  if (!srsSnapshotRef.current && srs) srsSnapshotRef.current = srs;
  const srsSnapshot = srsSnapshotRef.current || srs;
  const allItems = reactExports.useMemo(() => {
    const items = [
      ...vocabItems.map((it) => ({ ...it, srsBox: getSrsLevel(srsSnapshot, makeVocabKey(it.word)) })),
      ...kanjiItems.map((it) => ({ word: it.kanji, reading: it.on || it.kun || "", meaning: it.meaning || it.title || "", srsBox: getSrsLevel(srsSnapshot, makeKanjiKey(it.kanji)) }))
    ].filter((it) => it.word && it.meaning);
    return items.sort((a, b) => a.srsBox - b.srsBox);
  }, [vocabItems, kanjiItems]);
  if (allItems.length < 4) {
    return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "n4-empty-state", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "n4-empty-state-icon", children: "📭" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("p", { children: "Không đủ dữ liệu để ôn tập." })
    ] });
  }
  return /* @__PURE__ */ jsxRuntimeExports.jsx(
    QuizMode,
...
function WeakPoints({ vocabItems, kanjiItems }) {
  const bookmarks = useLearningStore((s) => s.bookmarks);
  const srs = useLearningStore((s) => s.srs);
  const srsSnapshotRef = reactExports.useRef(null);
  const bookmarksSnapshotRef = reactExports.useRef(null);
  if (!srsSnapshotRef.current && srs) srsSnapshotRef.current = srs;
  if (!bookmarksSnapshotRef.current && bookmarks) bookmarksSnapshotRef.current = bookmarks;
  const srsSnapshot = srsSnapshotRef.current || srs;
  const bookmarksSnapshot = bookmarksSnapshotRef.current || bookmarks;
  const weakItems = reactExports.useMemo(() => {
    const items = [
      ...vocabItems.map((it) => ({ ...it, weak: !!bookmarksSnapshot[makeVocabKey(it.word)] || getSrsLevel(srsSnapshot, makeVocabKey(it.word)) < 2 })),
      ...kanjiItems.map((it) => ({
        word: it.kanji,
        reading: it.on || "",
        meaning: it.meaning || it.title || "",
        weak: !!bookmarksSnapshot[makeKanjiKey(it.kanji)] || getSrsLevel(srsSnapshot, makeKanjiKey(it.kanji)) < 2
      }))
    ].filter((it) => it.word && it.meaning && it.weak);
    if (items.length < 4) {
      return [...vocabItems].sort(() => Math.random() - 0.5).slice(0, 10);
    }
    return items.sort(() => Math.random() - 0.5).slice(0, 15);
  }, [vocabItems, kanjiItems]);
  if (weakItems.length < 4) {
    return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "n4-empty-state", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "n4-empty-state-icon", children: "💪" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("p", { children: "Chưa có điểm yếu! Tiếp tục luyện tập." })
    ] });
  }
  return /* @__PURE__ */ jsxRuntimeExports.jsx(
    QuizMode,
    {
      ...
      getCorrectInfo: (it) => ({ reading: it.reading || "", meaning: it.meaning }),
      optionCount: 4,
      maxQuestions: Math.min(15, weakItems.length),
      speakOnShow: true
    }
  );
}
function DailyPractice({ mode = "today" }) {
  ...
  const renderMode = () => {
    ...
    switch (mode) {
      ...
      case "review":
        return /* @__PURE__ */ jsxRuntimeExports.jsx(ReviewMode, { vocabItems, kanjiItems });
      case "challenge": {
      ...
      case "weak-points":
        return /* @__PURE__ */ jsxRuntimeExports.jsx(WeakPoints, { vocabItems, kanjiItems });
      case "bookmarks":
