const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["./VoiceShadowing-8i1VucS4.js","./vendor-react-BUL8WuXG.js"])))=>i.map(i=>d[i]);
...
import { u as useScoreEngine, T as TrainerContext, c as calcXp } from "./useScoreEngine-f-lCg0MN.js";
import { c as createCombatText, a as createBattleBanner, g as getComboTier, G as GameplayEffects } from "./GameplayEffects-C1IXweFm.js";
import { a as QuizMode } from "./QuizMode-DQg4kJzy.js";
import { u as useVocabItems, c as useKanjiItems, b as useSectionList } from "./useDataHelper-DtTUT9Wk.js";
import { L as speakJP, b as stopSpeech, H as getQualityConfig, x as playSFX, a as useDataStore, o as onStopAll, h as useLearningStore, f as isMuted, c as speakLongText } from "./index-BEJSIlFS.js";
...
function normalizeDictationText(value) {
  return String(value || "").normalize("NFKC").trim().replace(/\s+/gu, "").replace(/[ァ-ヶ]/gu, (char) => String.fromCodePoint(char.codePointAt(0) - 96));
}
function sourceDictationReadings(item) {
  return [item == null ? void 0 : item.reading, ...Array.isArray(item == null ? void 0 : item.kunReadings) ? item.kunReadings : [], ...Array.isArray(item == null ? void 0 : item.onReadings) ? item.onReadings : []].flatMap((value) => String(value || "").replace(/\([^)]*\)/g, "").split(/[、,，/]/u)).map((value) => value.trim().replace(/\./g, "")).filter((value) => /[ぁ-ゖァ-ヶーｦ-ﾟ]/u.test(value));
}
function assessDictation(input, item) {
  const typed = normalizeDictationText(input);
  if (!typed) return false;
  return [item == null ? void 0 : item.word, ...sourceDictationReadings(item)].some((value) => value && normalizeDictationText(value) === typed);
}
function DictationMode({ items = [], maxQuestions = 15 }) {
  const pool = reactExports.useMemo(() => items.filter((item) => typeof item.word === "string" && item.word.trim()).slice(0, maxQuestions), [items, maxQuestions]);
  const scoring = useScoreEngine("listening-lab");
  const [index, setIndex] = reactExports.useState(0);
  const [typed, setTyped] = reactExports.useState("");
  const [feedback, setFeedback] = reactExports.useState(null);
  const [hint, setHint] = reactExports.useState(false);
  const [revealed, setRevealed] = reactExports.useState(false);
  const [finished, setFinished] = reactExports.useState(false);
  const [audioError, setAudioError] = reactExports.useState("");
  const assessed = reactExports.useRef(/* @__PURE__ */ new Set());
  const assisted = reactExports.useRef(/* @__PURE__ */ new Set());
  const correctCount = reactExports.useRef(0);
  const composing = reactExports.useRef(false);
  const answerLock = reactExports.useRef(false);
  const audioRequest = reactExports.useRef(0);
  const current = pool[index];
  const replayAudio = reactExports.useCallback(() => {
    if (!current) return;
    const request = ++audioRequest.current;
    setAudioError("");
    const failed = () => {
      if (audioRequest.current === request) setAudioError("Chưa phát được âm thanh. Bạn có thể bấm Nghe lại để thử lại.");
    };
    try {
      Promise.resolve(speakJP(current.word)).catch(failed);
    } catch (e) {
      failed();
    }
  }, [current]);
  ...
  reactExports.useEffect(() => {
    if (!finished && current) replayAudio();
    return stopAudio;
  }, [current, finished, replayAudio, stopAudio]);
  const check = (event) => {
    var _a;
    event.preventDefault();
    if (composing.current || ((_a = event.nativeEvent) == null ? void 0 : _a.isComposing) || answerLock.current || !typed.trim() || !current || revealed) return;
    answerLock.current = true;
    const correct = assessDictation(typed, current);
    if (!assessed.current.has(index) && !assisted.current.has(index)) {
      assessed.current.add(index);
      if (correct) {
  ...
  const restart = () => {
    ...
    assisted.current.clear();
    correctCount.current = 0;
    resetPrompt();
    setIndex(0);
    setFinished(false);
  };
  if (!pool.length) return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "n4-empty-state", children: "Không có nội dung để luyện chính tả." });
  if (finished) return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "n4-card", role: "status", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("h2", { children: "Đã hoàn thành lượt chính tả" }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("p", { children: [
      "Đúng ",
      correctCount.current,
      "/",
      assessed.current.size,
      " câu đã tự trả lời. Đã học ",
      pool.length,
      " mục."
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "n4-btn n4-btn-primary", onClick: restart, children: "Luyện lại" })
  ] });
  const reading = sourceDictationReadings(current)[0] || "";
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("section", { className: "n4-card n4-dictation-mode", "aria-label": "Luyện chính tả", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("header", { children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("h2", { children: "Nghe và viết lại" }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("p", { children: [
        "Câu ",
        index + 1,
        "/",
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsx("p", { children: "Nhập từ nghe được bằng kana hoặc kanji. Bạn có thể nghe lại và trả lời theo nhịp của mình." }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 12 }, children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "n4-btn n4-btn-primary", onClick: replayAudio, children: "🔊 Nghe lại" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "n4-btn n4-btn-ghost", onClick: () => setHint(true), children: "Gợi ý" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "n4-btn n4-btn-ghost", onClick: () => {
        assisted.current.add(index);
        setRevealed(true);
      }, children: "Xem đáp án" })
    ] }),
    audioError && /* @__PURE__ */ jsxRuntimeExports.jsx("p", { role: "status", children: audioError }),
    hint && /* @__PURE__ */ jsxRuntimeExports.jsxs("p", { children: [
      "Gợi ý: ",
      current.meaning || "Hãy nghe lại từ này.",
      reading && ` · Bắt đầu bằng ${Array.from(reading)[0]}`
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("form", { onSubmit: check, children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("label", { htmlFor: "dictation-answer", children: "Nội dung nghe được" }),
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "n4-btn n4-btn-primary", type: "submit", disabled: !typed.trim() || !!feedback || revealed, children: "Kiểm tra" })
    ] }),
    (feedback || revealed) && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { role: "status", style: { marginTop: 12 }, children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("p", { children: feedback === "correct" ? "Chính xác!" : feedback === "wrong" ? "Chưa đúng. Hãy đối chiếu đáp án và thử lại." : "Đã mở đáp án; mục này dùng để ôn, không tính là câu tự trả lời." }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("p", { lang: "ja", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("strong", { children: current.word }),
        reading && ` · ${reading}`
      ] }),
      current.meaning && /* @__PURE__ */ jsxRuntimeExports.jsx("p", { children: current.meaning }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "n4-btn n4-btn-ghost", onClick: () => {
        setFeedback(null);
        setRevealed(false);
        setTyped("");
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "n4-btn n4-btn-primary", onClick: next, children: index + 1 === pool.length ? "Hoàn thành" : "Tiếp →" })
    ] })
  ] });
}
function MechaRadioMode({ dictation = false, ...props }) {
  return dictation ? /* @__PURE__ */ jsxRuntimeExports.jsx(DictationMode, { ...props }) : /* @__PURE__ */ jsxRuntimeExports.jsx(MechaChoiceMode, { ...props });
}
function MechaChoiceMode({ items = [] }) {
  var _a;
...
const VoiceShadowing = reactExports.lazy(() => __vitePreload(() => import("./VoiceShadowing-8i1VucS4.js"), true ? __vite__mapDeps([0,1]) : void 0, import.meta.url));
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "n4-shadowing-mode", children: [
    ...
    record && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: `n4-shadowing-result ${record.ok ? "is-ok" : "is-off"}`, children: [
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "n4-shadowing-actions", children: [
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "n4-btn n4-btn-primary n4-btn-sm", onClick: next, children: idx + 1 >= pool.length ? "Hoàn thành" : "Tiếp →" })
      ] })
    ] })
  ] });
}
...
function ListeningLab({ mode = "dictation" }) {
  ...
  const renderMode = () => {
    switch (mode) {
      case "dictation":
        return /* @__PURE__ */ jsxRuntimeExports.jsx(MechaRadioMode, { items, dictation: true, maxQuestions: maxQ });
      case "shadowing":
