...
import { c as canonicalToolMistakes, a as canonicalToolDayStats, b as canonicalToolState, w as worldOverlayMount } from "./legacy-loader-gyvSYPc3.js";
import { ao as esc, ap as shuffleArray, au as selectDueSrsEntries, ai as S } from "./index-BEJSIlFS.js";
...
function openKanjiOfDay() {
  ...
  h += "</div>";
  var compounds = Array.isArray(k.compoundsList) ? k.compoundsList : Array.isArray(k.compounds) ? k.compounds : [];
  ...
  if (!compounds.length && typeof k.compounds === "string" && k.compounds.trim()) h += '<div style="white-space:pre-wrap">' + esc(k.compounds) + "</div>";
...
function openGrammarOfDay() {
  ...
  h += '<div style="color:var(--text-muted);font-size:0.8em;margin:4px 0">Mẫu ' + esc(String((_a = p.id) != null ? _a : "")) + "</div>";
  if (p.content) {
...
function openDailyMiniLesson() {
  ...
  _miniState = { answer: quizWord.meaning, item: quizWord, done: false, run: startLegacyToolAssessment("openDailyMiniLesson") };
  var h = '<div class="mg-container">';
...
function answerMiniLesson(idx) {
  ...
  submitLegacyToolAssessment("openDailyMiniLesson", _miniState.run, { questionId: "q0", item: _miniState.item, kind: "vocab", correct });
  finishLegacyToolAssessment("openDailyMiniLesson", _miniState.run);
...
function openDailyListening() {
  clearTimeout(_dlpTimer);
  var ol = _ov("daily-listen-overlay");
  var item = _dlpData[_dayNum() % _dlpData.length];
  _dlpState = { item, qIdx: 0, done: [], run: startLegacyToolAssessment("openDailyListening") };
  var h = '<div class="mg-container">';
...
function _dlpRenderQ() {
  ...
  if (c.qIdx >= c.item.qs.length) {
    finishLegacyToolAssessment("openDailyListening", c.run);
...
function answerDailyListening(idx) {
  ...
  submitLegacyToolAssessment("openDailyListening", c.run, { questionId: "q" + c.qIdx, correct });
  btns.forEach(function(b) {
    b.disabled = true;
  ...
  c.qIdx++;
  var run = c.run;
  _dlpTimer = setTimeout(function() {
    if (_dlpState.run === run && document.getElementById("dlp-qs")) _dlpRenderQ();
  }, 1500);
}
function closeDailyListening() {
  clearTimeout(_dlpTimer);
  finishLegacyToolAssessment("openDailyListening", _dlpState.run);
...
function openMistakeDigest() {
  ...
  var items = canonicalToolMistakes();
...
function openMorningQuiz() {
  ...
  vocabs.forEach(function(v) {
    ...
    qs.push({ type: "vocab", item: v, q: v.word, answer: v.meaning, opts: shuffleArray([v.meaning].concat(wrongs)) });
  });
  kanjis.forEach(function(k) {
    ...
    qs.push({ type: "kanji", item: k, q: k.kanji, answer: reading, opts: shuffleArray([reading].concat(wrongs)) });
  });
  var mixed = shuffleArray(qs).slice(0, 10);
  _mqState = { qs: mixed, idx: 0, score: 0, run: startLegacyToolAssessment("openMorningQuiz") };
  var h = '<div class="mg-container">';
...
function _mqRender() {
  ...
  if (c.idx >= c.qs.length) {
    finishLegacyToolAssessment("openMorningQuiz", c.run);
...
function answerMorningQuiz(idx) {
  ...
  submitLegacyToolAssessment("openMorningQuiz", c.run, { questionId: "q" + c.idx, item: q.item, kind: q.type, correct });
  btns.forEach(function(b) {
    b.disabled = true;
  ...
  c.idx++;
  var run = c.run;
  _mqTimer = setTimeout(function() {
    if (_mqState.run === run && document.getElementById("mq-body")) _mqRender();
  }, correct ? 800 : 1800);
}
function closeMorningQuiz() {
  clearTimeout(_mqTimer);
  finishLegacyToolAssessment("openMorningQuiz", _mqState.run);
  _clOv("morning-quiz-overlay");
}
function openNightReview() {
  var ol = _ov("night-review-overlay");
  var todayLog = canonicalToolDayStats();
  var today = todayLog.date;
  var canonical = canonicalToolState();
  var srsData = canonical.srs;
  var totalSrs = Object.keys(srsData).length;
  var dueCount = selectDueSrsEntries(srsData).length;
  var bmData = canonical.bookmarks || {};
  ...
  h += '<div style="text-align:center;padding:10px;background:var(--bg);border-radius:6px"><div style="font-size:1.5em;font-weight:700;color:var(--success)">' + todayLog.answers + '</div><div style="font-size:0.8em;color:var(--text-muted)">Câu trả lời</div></div>';
  h += "</div>";
