...
import "./QuizMode-DQg4kJzy.js";
import { K as equalsAnswerText, J as stableAnswerKey, o as onStopAll, b as stopSpeech, L as speakJP, x as playSFX } from "./index-BEJSIlFS.js";
import { u as useVocabItems, a as useGrammarItems } from "./useDataHelper-DtTUT9Wk.js";
import { u as useGrammarGameItems } from "./useGrammarQuiz-CkAk5tho.js";
import { g as getFirstExample } from "./index-B2ai8n3C.js";
...
function mockReadingPool(items) {
  return items.map((item) => {
    const example = getFirstExample(item);
    return { ...item, example: (example == null ? void 0 : example.jp) || item.example, exMeaning: (example == null ? void 0 : example.vi) || item.exMeaning };
  }).filter((item) => typeof item.example === "string" && item.example.trim() && typeof item.exMeaning === "string" && item.exMeaning.trim());
}
const MODES = [
...
function MockTest({ vocabItems, grammarItems, grammarGameItems, mode, scoring, onTimerChange }) {
  ...
  const questions = reactExports.useMemo(() => {
    ...
    const rPool = mockReadingPool(vocabItems).sort(() => Math.random() - 0.5);
