...
import { cy as PRIO, w as useMasteryStore, ae as loadDailyStudyPlan, h as useLearningStore, ba as GAME_EVENTS, cz as summarizeSets, cA as POWER_UP_META, cB as telemetry, cC as TELEMETRY_EVENTS, aN as PHASES, cD as getComboMultiplier, cE as COIN_PER_CORRECT, cF as SKILLS, cG as loadGemState, cH as saveGemState, cI as addGems, M as useQuestStore, a3 as getItemBonuses, cJ as getSetBonuses, cK as useWorldClockStore, b5 as completeBlockInStudyPlan, b6 as saveDailyStudyPlan, u as useAppStore, cL as useQuality, aJ as getStudyDateKey } from "./index-BEJSIlFS.js";
import { a as useNavigate, L as Link } from "./vendor-router-Dx6RIovR.js";
import { E as EMPTY_OBJ } from "./empty-Bvm-mx50.js";
import { u as useScreenShake, a as useHitPause } from "./HitPause-BapLYhfu.js";
import { i as isTechnicalAnswerEvent, a as snapshotMasteryBars, b as buildStudyResult } from "./study-results-DtyGPqxP.js";
import { Q as QUEST_CHAINS } from "./quest-chains-CiwzmCpJ.js";
import { b as getTrainerModeLabel } from "./registry-BAotxlgH.js";
...
function TimerBar({ seconds = 0, maxSeconds = 60, paused = false }) {
  ...
  const state2 = pct > 0.33 ? "safe" : pct > 0.1 ? "warn" : "danger";
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: `n4-timer-bar n4-timer-${state2}${paused ? " is-paused" : ""}`, role: "timer", "aria-label": `Thời gian ${formatMMSS(seconds)}`, children: [
...
function emit(event, payload) {
  ...
  const activeSessionId = payload && typeof payload === "object" && !Array.isArray(payload) ? (_a = getActiveSession()) == null ? void 0 : _a.sessionId : null;
  const correlatedPayload = activeSessionId && !payload.sessionId ? { ...payload, sessionId: activeSessionId } : payload;
  for (let i = 0; i < arr.length; i++) {
    try {
      arr[i].fn(correlatedPayload);
    } catch (err) {
...
function resolveAnswer({ correct, phase = PHASES.CORE, combo = 0, state: state2 = {}, itemBonuses, setBonuses, season }) {
  var _a, _b, _c, _d, _e;
  ...
  let additive = 1;
  if ((((_a = state2.powerUps) == null ? void 0 : _a.doubleCoins) || 0) > 0) additive += 1;
  if ((((_b = state2.activeEffects) == null ? void 0 : _b.goldRush) || 0) > 0) additive += 2;
  if ((_c = state2.activeEffects) == null ? void 0 : _c.coinMagnet) additive += 0.5;
  if (state2.vipExpiry && Date.now() < state2.vipExpiry) additive += 0.25;
  if (state2.activePet && ((_d = state2.pets) == null ? void 0 : _d[state2.activePet])) {
    const pet = state2.pets[state2.activePet];
...
function resolveArtifactTriggers({ equipped = {}, unlockedItems = {}, event = {}, state: state2 = {} } = {}) {
  const triggered = [];
...
function applyPayout(payout, context = {}) {
  var _a, _b;
  ...
  try {
    ...
    if (payout.gems > 0) {
      try {
        const state2 = loadGemState();
        saveGemState(addGems(state2, payout.gems, "game-reward"));
      } catch (err) {
...
function computeAnswerPayout({ event, state: state2, season, worldPhase } = {}) {
  var _a, _b;
  const payout = makeEmptyPayout();
  if (!event) return payout;
  const itemBonuses = getItemBonuses(state2) || {};
  const setBonuses = getSetBonuses(state2) || {};
  ...
  const triggered = resolveArtifactTriggers({
    equipped: state2 == null ? void 0 : state2.equippedCosmetics,
    unlockedItems: state2 == null ? void 0 : state2.unlockedItems,
    event,
    state: state2
  });
...
function computeSessionPayout({ summary, state: state2 } = {}) {
  const payout = makeEmptyPayout();
...
const completedSessionIds = /* @__PURE__ */ new Set();
function claimSessionFinish(sessionId) {
  if (!sessionId) return true;
  if (completedSessionIds.has(sessionId)) return false;
  completedSessionIds.add(sessionId);
  if (completedSessionIds.size > 500) {
    const oldest = completedSessionIds.values().next().value;
    completedSessionIds.delete(oldest);
  }
  return true;
}
...
function installRewardSubscriber() {
  ...
  bus.on(GAME_EVENTS.ANSWER_SUBMITTED, (event) => {
    try {
      if (isTechnicalAnswerEvent(event)) return;
      const state2 = useLearningStore.getState();
      const season = getSeasonSnapshot();
      const worldPhase = getWorldPhase();
      const payout = computeAnswerPayout({ event, state: state2, season, worldPhase });
      applyPayout(payout, { sessionId: event.sessionId });
    } catch (err) {
  ...
  bus.on(GAME_EVENTS.SESSION_FINISHED, (summary) => {
    try {
      if (!claimSessionFinish(summary.sessionId)) return;
      const state2 = useLearningStore.getState();
      const season = getSeasonSnapshot();
      const worldPhase = getWorldPhase();
      const payout = computeSessionPayout({ summary, state: state2, season, worldPhase });
      applyPayout(payout, { sessionId: summary.sessionId });
    } catch (err) {
...
function resolveAbilityEffect(petId) {
  var _a, _b, _c;
  try {
    const state2 = (_b = (_a = useLearningStore).getState) == null ? void 0 : _b.call(_a);
    const pet = (_c = state2 == null ? void 0 : state2.pets) == null ? void 0 : _c[petId];
...
function applyPetActive(abilityId) {
  ...
  if (abilityId.includes("coin-rain") || abilityId.includes("coin-burst")) {
    try {
      (_c = state2.addCoins) == null ? void 0 : _c.call(state2, 50, "pet-active");
    } catch (e) {
    }
    return;
  }
  if (abilityId.includes("royal-gift") || abilityId.includes("gift-hint")) {
    try {
      (_d = state2.grantPowerUp) == null ? void 0 : _d.call(state2, "hintPack", 1);
    } catch (e) {
    }
    return;
  }
  if (abilityId.includes("snow-cloak") || abilityId.includes("free-hint")) {
    try {
      (_e = state2.grantPowerUp) == null ? void 0 : _e.call(state2, "hintPack", 3);
    } catch (e) {
    }
    return;
  }
  ...
  if (abilityId.includes("bark-rally") || abilityId.includes("combo-plus")) {
    try {
      (_f = state2.addCombo) == null ? void 0 : _f.call(state2);
    } catch (e) {
    }
    return;
  }
  ...
  try {
    (_g = state2.addCoins) == null ? void 0 : _g.call(state2, 15, `pet-active:${abilityId}`);
  } catch (e) {
  }
}
function installPetActiveSubscriber() {
  ...
  bus.on(GAME_EVENTS.PET_ACTIVE_TRIGGERED, (payload) => {
    ...
    try {
      ...
      try {
        const state2 = (_b = (_a = useLearningStore).getState) == null ? void 0 : _b.call(_a);
        (_c = state2 == null ? void 0 : state2.setPetActiveCooldown) == null ? void 0 : _c.call(state2, petId, Date.now() + 900 * 1e3);
      } catch (e) {
      }
...
const ACTIVE_SESSION_MAX_AGE_MS = 4 * 60 * 60 * 1e3;
function isBlockMatching(block, trainerId, mode) {
  if (!block) return false;
  if (trainerId === "daily-practice") return block.route.includes(mode || "");
  const domainMap = {
    vocab: "vocab-dojo",
    kanji: "kanji-academy",
...
function isCurrentSession(payload) {
  return state.phase === "active" && !!(payload == null ? void 0 : payload.sessionId) && payload.sessionId === state.sessionId;
}
function clearPersistedSession(sessionId) {
  var _a;
  const latest = loadDailyStudyPlan();
  if (!latest || ((_a = latest.activeSession) == null ? void 0 : _a.sessionId) !== sessionId) return;
  saveDailyStudyPlan({ ...latest, activeSession: null });
}
function isPersistedSessionValid(plan, activeSession, now = Date.now()) {
  var _a;
  if (!plan || plan.status !== "active" || !(activeSession == null ? void 0 : activeSession.sessionId)) return false;
  if (activeSession.planId !== plan.id || activeSession.blockIndex !== plan.currentBlock) return false;
  if (((_a = plan.blocks[plan.currentBlock]) == null ? void 0 : _a.id) !== activeSession.blockId) return false;
  if (!activeSession.trainerId || !Number.isFinite(Number(activeSession.startedAt))) return false;
  const lastActivityAt = Number(activeSession.lastEventAt || activeSession.startedAt);
  return lastActivityAt <= now + 6e4 && now - lastActivityAt <= ACTIVE_SESSION_MAX_AGE_MS;
}
function persistActiveState({ allowSessionReplacement = false } = {}) {
  var _a, _b;
  if (state.phase !== "active") return null;
  const latest = loadDailyStudyPlan();
  if (!latest || latest.id !== state.planId || latest.currentBlock !== state.blockIndex || ((_a = latest.blocks[state.blockIndex]) == null ? void 0 : _a.id) !== state.blockId) {
    return null;
  }
  if (!allowSessionReplacement && ((_b = latest.activeSession) == null ? void 0 : _b.sessionId) !== state.sessionId) return null;
  return saveDailyStudyPlan({
    ...
    activeSession: {
      ...
      technicalFailures: state.technicalFailures.map((failure) => ({ ...failure })),
      domainBefore: state.domainBefore,
      rewards: { ...state.rewards }
    }
  });
}
function restoreStudySessionFromPlan(now = Date.now()) {
  var _a, _b, _c;
  const plan = loadDailyStudyPlan(now);
  const activeSession = plan == null ? void 0 : plan.activeSession;
  if (!activeSession) {
    idle();
    return getStudySessionState();
  }
  if (!isPersistedSessionValid(plan, activeSession, now)) {
    saveDailyStudyPlan({ ...plan, activeSession: null });
    idle();
    return getStudySessionState();
  }
  state = {
    ...
    blockIndex: activeSession.blockIndex,
    blockId: activeSession.blockId,
    trainerId: activeSession.trainerId,
    mode: activeSession.mode || void 0,
    startedAt: Number(activeSession.startedAt),
    answers: Array.isArray(activeSession.answers) ? activeSession.answers.map((answer) => ({ ...answer })) : [],
    technicalFailures: Array.isArray(activeSession.technicalFailures) ? activeSession.technicalFailures.map((failure) => ({ ...failure })) : [],
    domainBefore: snapshotMasteryBars(activeSession.domainBefore),
    rewards: {
      xp: Number((_a = activeSession.rewards) == null ? void 0 : _a.xp) || 0,
      coins: Number((_b = activeSession.rewards) == null ? void 0 : _b.coins) || 0,
      gems: Number((_c = activeSession.rewards) == null ? void 0 : _c.gems) || 0
    }
  };
  return getStudySessionState();
}
function cancelActive(reason = "cancelled") {
  ...
  state = { ...state, phase: "cancelled", reason };
  clearPersistedSession(sessionId);
  idle();
}
function getOrCreateAnswer(itemKey, timestamp = Date.now()) {
  let answer = state.answers.find((entry) => entry.itemKey === itemKey);
  if (!answer) {
    answer = {
      itemKey,
...
function recordHelperUse(event, field) {
  var _a, _b;
  if (state.phase !== "active") return;
  if (!((_a = event.detail) == null ? void 0 : _a.sessionId) || event.detail.sessionId !== state.sessionId) return;
  const itemKey = (_b = event.detail) == null ? void 0 : _b.itemKey;
  if (!itemKey) return;
  getOrCreateAnswer(itemKey)[field] = true;
  persistActiveState();
}
function getStudySessionState() {
  ...
  return {
    ...state,
    answers: ((_a = state.answers) == null ? void 0 : _a.map((answer) => ({ ...answer }))) || [],
    rewards: state.rewards ? { ...state.rewards } : void 0
  };
}
function installStudySessionSubscriber() {
  ...
  installed = true;
  if (typeof window !== "undefined") {
    window.addEventListener("n4:study-os:hint-used", (event) => recordHelperUse(event, "hintUsed"));
    window.addEventListener("n4:study-os:reveal-used", (event) => recordHelperUse(event, "revealUsed"));
  }
  restoreStudySessionFromPlan();
  bus.on(GAME_EVENTS.SESSION_STARTED, (event = {}) => {
    try {
      if (!event.sessionId) return;
      const plan = loadDailyStudyPlan();
      if (!plan || plan.status !== "active") return;
      const blockIndex = plan.currentBlock;
      const block = plan.blocks[blockIndex];
      if (!block || !isBlockMatching(block, event.trainerId, event.mode)) return;
      const canResumeRestored = state.phase === "active" && state.restored && state.planId === plan.id && state.blockIndex === blockIndex && state.blockId === block.id && state.trainerId === event.trainerId && (state.mode || null) === (event.mode || null);
      if (canResumeRestored) {
        state = {
          ...state,
          restored: false,
          sessionId: event.sessionId,
          startedAt: Number(state.startedAt) || event.startedAt || Date.now()
        };
        persistActiveState({ allowSessionReplacement: true });
        return;
      }
      ...
      state = {
        ...
        answers: [],
        technicalFailures: [],
        domainBefore: snapshotMasteryBars(useMasteryStore.getState().masteryBars),
        rewards: { xp: 0, coins: 0, gems: 0 }
      };
      persistActiveState({ allowSessionReplacement: true });
    } catch (error) {
      console.warn("[study-session-subscriber] start", error);
      cancelActive("errored");
    }
  }, { prio: PRIO.PROGRESSION, tag: "study-session" });
  bus.on(GAME_EVENTS.ANSWER_SUBMITTED, (event = {}) => {
    var _a, _b;
    if (!isCurrentSession(event) || !event.itemKey) return;
    try {
      if (isTechnicalAnswerEvent(event)) {
        state.technicalFailures.push({
          itemKey: event.itemKey,
          reason: ((_a = event.meta) == null ? void 0 : _a.reason) || event.technicalFailure || ((_b = event.meta) == null ? void 0 : _b.technicalFailure) || "technical-error",
          at: event.ts || Date.now()
        });
        persistActiveState();
        return;
      }
      const answer = getOrCreateAnswer(event.itemKey, event.ts || Date.now());
      if (event.itemKind) answer.itemKind = event.itemKind;
      const correct = !!event.correct;
      if (answer.firstAttemptCorrect == null) answer.firstAttemptCorrect = correct;
      answer.attempts += 1;
      answer.eventualCorrect || (answer.eventualCorrect = correct);
      answer.correct = answer.eventualCorrect;
      if (event.quality != null) answer.quality = event.quality;
      const learning = useLearningStore.getState();
      if (correct && answer.firstAttemptCorrect === false) {
        learning.markMistakeCorrectedInSession(event.itemKey);
      } else if (!correct && answer.attempts === 1) {
        learning.recordMistake(event.itemKey);
      }
      persistActiveState();
    } catch (error) {
      console.warn("[study-session-subscriber] answer", error);
    }
  }, { prio: PRIO.PROGRESSION, tag: "study-session" });
  bus.on(GAME_EVENTS.REWARD_DISPATCHED, (receipt = {}) => {
    if (!isCurrentSession(receipt)) return;
    state.rewards.xp += Number(receipt.xp) || 0;
    state.rewards.coins += Number(receipt.coins) || 0;
    state.rewards.gems += Number(receipt.gems) || 0;
    persistActiveState();
  }, { prio: PRIO.PROGRESSION, tag: "study-session" });
  bus.on(GAME_EVENTS.SESSION_CANCELLED, (event = {}) => {
    if (isCurrentSession(event)) cancelActive(event.reason || "cancelled");
  }, { prio: PRIO.PROGRESSION, tag: "study-session" });
  bus.on(GAME_EVENTS.SESSION_FINISHED, (summary = {}) => {
    var _a, _b;
    if (!isCurrentSession(summary)) return;
    const session = state;
    state = { ...state, phase: "completed" };
    try {
      const latest = loadDailyStudyPlan();
      if (!latest || latest.id !== session.planId || latest.currentBlock !== session.blockIndex || ((_a = latest.blocks[session.blockIndex]) == null ? void 0 : _a.id) !== session.blockId || ((_b = latest.activeSession) == null ? void 0 : _b.sessionId) !== session.sessionId) {
        clearPersistedSession(session.sessionId);
        return;
      }
      const result = buildStudyResult({
        answers: session.answers,
        technicalFailures: session.technicalFailures,
        summary,
        rewards: session.rewards,
        sessionId: session.sessionId,
        domainBefore: session.domainBefore,
        domainAfter: snapshotMasteryBars(useMasteryStore.getState().masteryBars)
      });
      completeBlockInStudyPlan(
        latest,
        session.blockIndex,
        session.answers,
        result,
      ...
      console.warn("[study-session-subscriber] finish", error);
      state = { ...state, phase: "errored" };
    } finally {
      idle();
    }
  }, { prio: PRIO.PROGRESSION, tag: "study-session" });
...
function createSessionId(trainerId) {
  var _a, _b;
...
function useGameSession({
  ...
  const statusRef = reactExports.useRef(STATES.IDLE);
  reactExports.useEffect(() => {
  ...
  reactExports.useEffect(() => () => {
    if (sessionIdRef.current && [STATES.RUNNING, STATES.PAUSED].includes(statusRef.current)) {
      bus.emit(GAME_EVENTS.SESSION_CANCELLED, {
  ...
  const emitPhaseIfChanged = reactExports.useCallback((nextPhase, idx) => {
    ...
    if (nextPhase !== phaseRef.current) {
      ...
      setPhase(nextPhase);
      const payload = { sessionId: sessionIdRef.current, phase: nextPhase, idx, of: (_a = rhythmRef.current) == null ? void 0 : _a.totalItems, trainerId, mode };
  ...
  const begin = reactExports.useCallback(() => {
    ...
    sessionIdRef.current = createSessionId(trainerId);
    statusRef.current = STATES.RUNNING;
    ...
    telemetry.emit(TELEMETRY_EVENTS.GAME_SESSION_STARTED, { sessionId: sessionIdRef.current, trainerId, mode, at: Date.now() });
    bus.emit(GAME_EVENTS.SESSION_STARTED, { sessionId: sessionIdRef.current, trainerId, mode, startedAt: Date.now(), rhythm: rhythmRef.current });
    bus.emit(GAME_EVENTS.SESSION_PHASE_ENTERED, { sessionId: sessionIdRef.current, phase: PHASES.WARMUP, idx: 0, of: (_a = rhythmRef.current) == null ? void 0 : _a.totalItems, trainerId, mode });
  }, [startHearts, hazardSeconds, trainerId, mode]);
  const finish = reactExports.useCallback((extra = {}) => {
    if (statusRef.current === STATES.FINISHED) return;
    statusRef.current = STATES.FINISHED;
  ...
  }, [answers, events, hp, mode, onFinish, trainerId]);
  reactExports.useEffect(() => {
  ...
  const pause = reactExports.useCallback(() => {
    setStatus((current2) => {
      const next = current2 === STATES.RUNNING ? STATES.PAUSED : current2;
  ...
  const resume = reactExports.useCallback(() => {
    setStatus((current2) => {
      const next = current2 === STATES.PAUSED ? STATES.RUNNING : current2;
      statusRef.current = next;
      return next;
    });
  }, []);
  const submitAnswer = reactExports.useCallback(({ correct, itemKey, itemKind, quality, meta } = {}) => {
    ...
    const entry = { sessionId: sessionIdRef.current, correct: !!correct, itemKey, itemKind, quality, ts, meta, combo: comboAfter, phase: phaseRef.current };
    setAnswers((arr) => {
    ...
    if (correct) {
      bus.emit(GAME_EVENTS.COMBO_CHANGED, { sessionId: sessionIdRef.current, combo: comboAfter, delta: 1, phase: phaseRef.current });
      const tier = comboAfter >= 25 ? 4 : comboAfter >= 10 ? 3 : comboAfter >= 5 ? 2 : comboAfter >= 1 ? 1 : 0;
      if (tier >= 2) bus.emit(GAME_EVENTS.COMBO_TIER_UP, { sessionId: sessionIdRef.current, tier, combo: comboAfter });
      const correctTotal = answers.filter((a) => a.correct).length + 1;
      if (correctTotal % 10 === 0) bus.emit(GAME_EVENTS.MILESTONE_HIT, { sessionId: sessionIdRef.current, correctTotal });
    } else {
      bus.emit(GAME_EVENTS.COMBO_BROKEN, { sessionId: sessionIdRef.current, phase: phaseRef.current });
    }
  ...
  const damage2 = reactExports.useCallback((amount = 1) => {
    setHp((h) => {
      ...
      bus.emit(GAME_EVENTS.HP_CHANGED, { sessionId: sessionIdRef.current, hp: next, delta, cause: "damage" });
  ...
  const heal2 = reactExports.useCallback((amount = 1) => {
    setHp((h) => {
      ...
      bus.emit(GAME_EVENTS.HP_CHANGED, { sessionId: sessionIdRef.current, hp: next, delta, cause: "heal" });
      return next;
    });
  }, [startHearts]);
...
function TrainerTopBar({ trainerId, activeMode }) {
  const studyActivity = useLearningStore((state2) => state2.studyActivity || EMPTY_ACTIVITY);
  const activity = studyActivity[getStudyDateKey()] || EMPTY_ACTIVITY;
  const answerCount = Math.max(0, Number(activity.total) || 0);
  const sessionCount = Math.max(0, Number(activity.sessions) || 0);
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("aside", { className: "n4-trainer-guidance", "aria-label": "Bài luyện đang thực hiện", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "n4-trainer-guidance__label", children: "Bài đang gợi ý" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("strong", { children: getTrainerModeLabel(activeMode) })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("p", { children: answerCount > 0 ? `Đã ghi nhận ${answerCount} câu${sessionCount ? ` · ${sessionCount} phiên` : ""} hôm nay.` : "Kết quả được lưu ngay sau câu trả lời đầu tiên." }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "n4-trainer-guidance__trainer", "aria-hidden": "true", children: trainerId })
  ] });
}
export {
  GameShell as G,
