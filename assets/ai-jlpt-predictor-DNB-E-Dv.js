...
import { h as useLearningStore, a as useDataStore, _ as getSrsLevel, ax as getLearningKeyType } from "./index-BEJSIlFS.js";
...
function useJLPTScores() {
  ...
  return reactExports.useMemo(() => {
    ...
    const srsKanjiMastered = srsKanji.filter(([, state]) => getSrsLevel(state) >= 3).length;
    const srsVocabMastered = srsVocab.filter(([, state]) => getSrsLevel(state) >= 3).length;
    const srsGrammarMastered = srsGrammar.filter(([, state]) => getSrsLevel(state) >= 3).length;
    const catAcc = (ids) => {
      let c = 0, t = 0;
      ids.forEach((id) => {
