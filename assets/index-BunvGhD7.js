const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["./vendor-three-webgpu-6wEeObXc.js","./vendor-three-Ba7Uoy0A.js"])))=>i.map(i=>d[i]);
...
import { c as createCombatText, a as createBattleBanner, g as getComboTier, G as GameplayEffects, b as getBossMood } from "./feature-particles-BSY69iaM.js";
import { F as create, bn as getQualityConfig, _ as __vitePreload, bA as playSFX, az as stableAnswerKey, aA as equalsAnswerText, ac as onStopAll, a5 as stopSpeech, f as speakJP } from "./feature-3d-CFvJkEt3.js";
import { u as useVocabItems, a as useKanjiItems, b as useGrammarItems, c as useSectionList } from "./useDataHelper-CLs8Rj_F.js";
...
function WebGPUCanvas({ onRendererReady, className = "", style }) {
  ...
  const init = reactExports.useCallback(async () => {
    ...
    try {
      const THREE = await __vitePreload(() => import("./vendor-three-webgpu-6wEeObXc.js").then((n) => n.t), true ? __vite__mapDeps([0,1]) : void 0, import.meta.url);
      ...
      try {
        const { WebGLRenderer } = await __vitePreload(async () => {
          const { WebGLRenderer: WebGLRenderer2 } = await import("./vendor-three-Ba7Uoy0A.js").then((n) => n.T);
    ...
    const { Scene, PerspectiveCamera } = await __vitePreload(async () => {
      const { Scene: Scene2, PerspectiveCamera: PerspectiveCamera2 } = await import("./vendor-three-Ba7Uoy0A.js").then((n) => n.T);
...
async function createSlashParticles(count = 1e4) {
  ...
  const { storage, instanceIndex, Fn, color, pass } = await __vitePreload(async () => {
    const { storage: storage2, instanceIndex: instanceIndex2, Fn: Fn2, color: color2, pass: pass2 } = await import("./vendor-three-webgpu-6wEeObXc.js").then((n) => n.a);
